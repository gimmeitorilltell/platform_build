...
#  default off if unset.  The candy_config.mk is included in:
#    $(BUILD_SYSTEM)/config.mk

# current build configurations:
# BONE_STOCK := set true to override all CANDY_config variables
...
# CANDY_ARM_OPT_LEVEL := -Ox for TARGET_arm_CFLAGS, preserved in binary.mk
# CANDY_THUMB_OPT_LEVEL := -Ox for TARGET_thumb_CFLAGS, preserved in binary.mk
...
# Assign modules to build with link time optimizations using CANDY_LTO_MODULES.
ifeq ($(USE_LTO),true)
  CANDY_LTO_MODULES := \
    core.art \
    libart \
...
# fstrict-aliasing. Thumb is defaulted off for AOSP. Use CANDY_SPECIAL_CASE_MODULES to
...
ifeq ($(ADDITIONAL_TARGET_THUMB_OPT),true)
    CANDY_TARGET_THUMB_FLAGS := \
        -funsafe-math-optimizations
endif

# Additional clang-specific cflags
ifeq ($(USE_EXTRA_CLANG_FLAGS),true)
    CANDY_CLANG_CONFIG_EXTRA_ASFLAGS :=
    CANDY_CLANG_CONFIG_EXTRA_CFLAGS :=
    CANDY_CLANG_CONFIG_EXTRA_CPPFLAGS :=
    CANDY_CLANG_CONFIG_EXTRA_LDFLAGS :=
endif

#======================================================================================================
# variables as exported to other makefiles ============================================================
CANDY_FSTRICT_OPTIONS := $(FSTRICT_FLAGS)

CANDY_GLOBAL_CFLAGS += $(DEBUG_SYMBOL_FLAGS) $(DEBUG_FRAME_POINTER_FLAGS)
CANDY_RELEASE_CFLAGS += $(DEBUG_SYMBOL_FLAGS) $(DEBUG_FRAME_POINTER_FLAGS)
CANDY_CLANG_TARGET_GLOBAL_CFLAGS += $(DEBUG_SYMBOL_FLAGS) $(DEBUG_FRAME_POINTER_FLAGS)
CANDY_GLOBAL_CPPFLAGS += $(DEBUG_SYMBOL_FLAGS) $(DEBUG_FRAME_POINTER_FLAGS)
...
CANDY_BINARY_CFLAG_OPTIONS := $(GRAPHITE_FLAGS)
CANDY_BINARY_CPP_OPTIONS := $(GRAPHITE_FLAGS)
CANDY_LINKER_OPTIONS :=
CANDY_ASSEMBLER_OPTIONS :=
endif
